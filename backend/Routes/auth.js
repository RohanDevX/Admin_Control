...
const jwt      = require('jsonwebtoken'); // <-- added: jwt is used in refresh verification/rotation
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    // NOTE: SameSite set to 'lax' for non-production (local dev & different port)
    // In production we keep 'strict' for maximum CSRF protection.
...
router.post('/refresh', async (req,res,next) => {
  try {
    ...
    // NOTE: use 'lax' during development to allow cross-site cookie refresh from localhost:3000
